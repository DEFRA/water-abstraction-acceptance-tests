...
Data files are being migrated, one file at a time, from an older shape to a newer one. Check which shape a given file is on before editing it — the two coexist in the repo during the migration.

- **Legacy shape**: a key holds an array containing a single item, e.g. `licences: [{ id, licenceRef, ... }]`, because the seed endpoint's wire format requires every key to be an array. Consumers destructure the single item out with an array pattern (`const { licences: [licence] } = licenceData`).
- **Migrated shape**: a key holds the item directly, e.g. `licences: { id, licenceRef, ... }`. A data file only migrates to this shape once all of its callers are updated in the same change — don't return a single object from a data file while a caller still expects an array, or vice versa.
- **Migrated single-entity data file**: a data file that builds exactly one entity (the normal case once multi-entity files are split — see below) returns the entity itself, with no table-name wrapper at all, e.g. `licenceData` returns `{ id, licenceRef, ... }`, not `{ licences: { id, licenceRef, ... } }`. Wrapping a single return value in its own table name forces every caller into a needless `.tableName` dot-access or destructure to reach the entity. A data file that genuinely builds more than one entity (e.g. `company.data.js` producing `companies`/`addresses`/`companyAddresses`, or `licence-version-purpose.data.js` producing `licenceVersionPurposes`/`licenceVersionPurposePoints`) keeps its table-name-keyed object, since there's no single bare value to return — see the split rule below for whether that should change instead.
- **Compose with the singular entity name as the key, not the plural table name.** Where something merges bare single-entity results into the seeding object — a scenario's `_licence`-style private helper, or the outer scenario function itself — use the singular name as the key via object shorthand, e.g. `return { permitLicence, licenceDocument, licence, licenceVersion }`, not `{ permitLicences: permitLicence, licenceDocuments: licenceDocument, licences: licence, licenceVersions: licenceVersion }`. Spec files destructure the same singular key back out, e.g. `const { licence: scenarioLicence } = scenario`. Nothing composing a scenario needs to know or spell out the plural table name — see the next bullet for how that gets resolved.
- Regardless of shape, the wire format sent to the seed endpoint must still be pluralized, array-valued keys (e.g. `licences: [{...}]`). This is handled once, centrally, in the `load` fixture (`tests/support/fixtures.js`)'s `_asArrays` helper, which — for every key in the final scenario object — pluralizes it if it doesn't already end in `s` (`licence` → `licences`, `point` → `points`) and wraps the value in an array if it isn't one already. Scenario and data files never do either of these themselves, and never need to; a single scenario's final data object can freely mix singular and already-plural keys, and single-object and array values — `_asArrays` normalizes all of it in one pass.
- A data file on the migrated shape takes the specific entity object(s)/array(s) it needs as parameters, not the whole upstream `*Data` result object — see the "Data file parameters (single-object data files)" rule in `.agents/skills/standards/SKILL.md`. The calling scenario (or data) file extracts each entity via direct property access at the call site, e.g. `licenceData(licenceRef, company.companies, company.addresses)`, rather than handing over the whole `company` result for the callee to unpack. Once an entity is built, pass that whole entity into further builders that need it — don't extract its individual fields into separate scalar arguments (e.g. `permitLicenceData(licence)`, not `permitLicenceData(licence.licenceRef, licence.startDate)`).
- Migrating a data file to the new shape means also updating every one of its direct callers (other data files, scenario files, and any spec files that destructure its result) in the same piece of work — a half-migrated file breaks its callers. It's acceptable for that follow-on work to temporarily break scenarios/specs further down the dependency chain that aren't part of the current migration step; those get fixed when their turn comes.
- A data file that returns several unrelated entities under one function (e.g. one `licence.data.js` producing `permitLicences`, `licenceDocumentHeaders`, `licenceDocuments`, `licenceDocumentRoles`, `licences`, and `licenceVersions`) should be split so each entity gets its own single-purpose `*.data.js` file, named and parameterised for that entity alone. This lets other scenarios reuse just the one entity they need (e.g. only `licence-version.data.js`) instead of being forced to take the whole bundle.
- The composition that used to live inside that one big data file moves to a private helper in the scenario file that first needed it, named after the thing it builds (e.g. `_licence` in `licence.scenario.js`, composing `licenceData`, `permitLicenceData`, `licenceDocumentData`, `licenceDocumentHeaderData`, `licenceDocumentRoleData`, and `licenceVersionData`). This gives other scenarios a single reusable building block for "what a core licence looks like" without duplicating the composition logic, while still allowing a scenario that only needs one piece (e.g. just a licence version) to call that entity's data file directly.
