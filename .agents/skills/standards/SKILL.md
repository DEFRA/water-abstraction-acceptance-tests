...
- Prefer role- and label-based locators (`getByRole`, `getByLabel`, `getByText`) over positional or structural CSS selectors like `:nth-child(n)` or `.nth(n)` — target what an element means (its role, label, or `data-test` attribute), not where it sits in the DOM
