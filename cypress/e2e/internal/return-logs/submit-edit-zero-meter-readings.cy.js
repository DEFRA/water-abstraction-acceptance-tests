...
describe('Submit then edit a meter readings return with zero reading (internal)', () => {
  beforeEach(() => {
    cy.tearDown()
  ...
  it('submit a return and check that the zero values recorded are correctly carried over when editing', () => {
    ...
    cy.get('[data-test="monthly-total-0"]').contains('0')
    ...
    cy.get('.govuk-heading-l').contains('Water abstracted April 2020')
    cy.get('.govuk-label').contains('April 2020')
    cy.get('#April2020').should('have.value', '0')
  })
})
