Given("I navigate to the {string} section") do |section|
  ...
  @page.click_manage_tab
  expect(@page.heading).to have_text("Manage reports and notices")

end


When("I create a {string} bill run for {string} region") do |bill_run_type, region|
...
Then("I can see the {string}") do |bill_action_type|
  if bill_action_type.eql? 'bill run started'
    ...
    expect(page.h1_heading).to have_text("Bill run exists")
