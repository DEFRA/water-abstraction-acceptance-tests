...
Then(/^I can see the waiting page$/) do
  ...
  url_regex = /^https:\/\/([a-z.-]+)\/waiting\/\w{8}-\w{4}-\w{4}-\w{4}-\w{12}$/
...
Then(/^I can confirm sending the returns invitations$/) do
  @waiting_page.wait_until_waiting_page_message_invisible
  @send_returns_invitations = Pages::Internal::Manage::SendReturnsInvitations.new
  url_regex = /^https:\/\/([a-z.-]+)\/batch-notifications\/review\/\w{8}-\w{4}-\w{4}-\w{4}-\w{12}$/
...
Then(/^I can see the returns invitations "([^"]*)" page$/) do |page_type|
