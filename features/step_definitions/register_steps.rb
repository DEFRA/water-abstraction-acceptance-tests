...
Given(/^I receive an email with sign in details$/) do
  ...
  within_window @new_window do
    @front_app.reg20_create_pw_page.submit(
...
Then(/^I am on the add licences page$/) do
  expect(@front_app.reg30_add_licences_page.current_url).to include "/add-licences"
  expect(@front_app.reg30_add_licences_page.heading).to have_text("Which licences do you want to be able to view?")
end

When(/^I register a licence$/) do
  @licence_reg = Quke::Quke.config.custom["data"][@environment]["licence_reg"]
  @front_app.reg30_add_licences_page.submit(
    licence_box: @licence_reg
  )
  @front_app.reg40_confirm_licences_page.submit
  @front_app.reg50_choose_address_page.submit
end

When(/^I receive a confirmation code$/) do
  @security_code = @front_app.reg60_sending_letter_page.security_code.text
  puts "Confirmation code is: " + @security_code
end

When(/^I am on the confirmation code page$/) do
  expect(@front_app.reg70_security_code_page.current_url).to include "/security-code"
end

When(/^I enter my confirmation code$/) do
  @front_app.reg70_security_code_page.submit(
