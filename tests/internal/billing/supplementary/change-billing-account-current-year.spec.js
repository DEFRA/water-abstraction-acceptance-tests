...
test.describe(
  ...
  () => {
    ...
    test('sends the sroc supplementary bill run, changes the billing account within the current financial year, then confirms the credit and new bill', async ({
      page
    }) => {
      ...
      await expect(page.locator('[data-test="region-1"]')).toContainText(regions.NORTH_EAST.displayNam)
      ...
      await expect(page.locator('h1')).toContainText(`${regions.NORTH_EAST.displayNam} supplementary`)
      ...
      // The 1st of April at the start of the current financial year
      await page.locator('#customDate-day').fill('1')
      await page.locator('#customDate-month').fill('4')
      ...
      await expect(page.locator('h1')).toContainText('Select the region')
      await page.getByRole('radio', { name: regions.NORTH_EAST.displayNam }).check()
      await page.getByRole('button', { name: 'Continue' }).click()
      ...
      await expect(page.locator('h1')).toContainText(`${regions.NORTH_EAST.displayNam} supplementary`)
      await expect(page.locator('#main-content > p > .govuk-tag')).toContainText('ready')
      await expect(page.locator('[data-test="bills-count"]')).toContainText('2 Supplementary bills')
      await expect(page.locator('[data-test="credits-count"]')).toContainText('1 credit note')
      await expect(page.locator('[data-test="debits-count"]')).toContainText('1 invoice')

      const abstractorsTable = page.locator('[data-test="other-abstractors"]')
      const secondCompanyRow = abstractorsTable.getByRole('row', { name: secondCompany.name })

      await expect(secondCompanyRow).toContainText(licence.licenceRef)

      const originalCompanyRow = abstractorsTable.getByRole('row', { name: billingAccount.accountNumber })

      await expect(originalCompanyRow).toContainText(licence.licenceRef)
    })
  }
)
