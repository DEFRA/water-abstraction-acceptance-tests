...
test.describe(
  'Cancelling a workflow item (internal)',
  {
    ...
    annotation: {
      type: 'info',
      description:
        'When the the licence is in workflow, and you cancel a workflow item and there is a two-part tariff bill run, and the workflow was created during the billing period, the licence should be flagged for supplementary billing' +
        '/n A two-part tariff bill run is created for the two-part tariff year, which is the year before the annual bill run. The workflow created at date is set to a date within the two-part tariff year.'
    }
  },
  () => {
    let company
